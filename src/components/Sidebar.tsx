  ...
  return (
    <div className="flex flex-col h-full bg-card/60 backdrop-blur-md border-r border-border/10 p-4">
      ...
      <button
        ...
        disabled={creating}
        className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-primary text-primary-foreground font-medium text-sm hover:opacity-95 transition-all shadow-sm shadow-primary/10 active:scale-98 cursor-pointer disabled:opacity-50 disabled:scale-100 animate-fade-in"
      >
        {creating ? (
      ...
      <div className="relative mt-3 px-2">
        <Search className="absolute left-5 top-2.5 h-3.5 w-3.5 text-muted-foreground/40" />
        <input
          type="text"
          placeholder="Search chats..."
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
          className="w-full pl-9 pr-7 py-2 text-xs rounded-xl border border-border bg-background/20 focus:outline-none focus:ring-1 focus:ring-primary/40 focus:border-primary text-foreground placeholder:text-muted-foreground/40 transition-all"
        />
        {searchQuery && (
          <button
            onClick={() => setSearchQuery('')}
            className="absolute right-4 top-2.5 p-0.5 rounded hover:bg-muted text-muted-foreground/60 hover:text-foreground cursor-pointer"
            type="button"
          >
            <X className="w-3 h-3" />
          </button>
